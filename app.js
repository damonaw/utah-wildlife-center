...
const Contact = { template: '<section class="content-section"><div class="container"><h2>Contact Us</h2><form @submit.prevent="submitForm"><label for="name">Name:</label><input type="text" id="name" name="name" required><label for="email">Email:</label><input type="email" id="email" name="email" required><label for="message">Message:</label><textarea id="message" name="message" required></textarea><button type="submit">Send</button></form><div id="map"></div></div></section>', methods: { submitForm() { const name = document.getElementById('name').value; const email = document.getElementById('email').value; const message = document.getElementById('message').value; window.location.href = `mailto:info@utahwildlifecenter.org?subject=Contact Form Submission&body=Name: ${name}%0AEmail: ${email}%0AMessage: ${message}`; } } };
const Newsletter = { template: '<section class="content-section"><div class="container"><h2>Newsletter Signup</h2><form @submit.prevent="submitForm"><label for="newsletter-email">Email:</label><input type="email" id="newsletter-email" name="newsletter-email" required><button type="submit">Sign Up</button></form></div></section>', methods: { submitForm() { const email = document.getElementById('newsletter-email').value; window.location.href = `mailto:info@utahwildlifecenter.org?subject=Newsletter Signup&body=Please add ${email} to the newsletter list.`; } } };
